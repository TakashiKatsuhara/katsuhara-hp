...
(function(){
  ...
  /* ---- hero photo subtle parallax (desktop only) ---- */
  const heroPhoto = document.querySelector('.hero .photo');
  const heroBlobs = document.querySelectorAll('.hero .blob-a, .hero .blob-b, .hero .blob-c');
  if (heroPhoto || heroBlobs.length){
    window.addEventListener('scroll', () => {
      const y = window.scrollY;
      const isDesktop = window.innerWidth > 1024;
      if (y < 800 && isDesktop){
        if (heroPhoto) heroPhoto.style.transform = `translateY(${y * 0.08}px)`;
        heroBlobs.forEach((b,i) => b.style.transform = `translateY(${y * (0.05 + i*0.03)}px)`);
      }
    }, {passive:true});
  }
})();
